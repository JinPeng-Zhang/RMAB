...
def getR(queuenum,STATE_MAX,weight,e,pcome,pcomee):
    ...
    for q in range(queuenum):
        ...
        R[q][STATE_MAX - 1][0] = -STATE_MAX*weight*pcome[q]
        R[q][STATE_MAX - 1][1] = -STATE_MAX*weight*pcome[q]
    return R
# print(R)
...
for i in range(enum):
    lam = []
    ee = ebegin+i*0.01 #本轮wt计算的e值
    R = getR(queuenum,STATE_MAX,weight,ee,pcome,pcomee)
    file_path = 'C:/Users/48935/Desktop/test/' + 's=' + str(STATE_MAX) + '  e=' + str(ee) + '.txt'
    print(ee)
    for q in range(queuenum):
        lam.append([])
